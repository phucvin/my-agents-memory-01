...
function username$moonbit_examples$cmd$factorial$$factorial(n) {
  let result = 1;
  let _tmp = 1;
  while (true) {
    const i = _tmp;
    if (i <= n) {
      result = Math.imul(result, i) | 0;
      _tmp = i + 1 | 0;
      continue;
    } else {
      break;
...
(() => {
  moonbitlang$core$builtin$$println$5$(`Factorial of ${moonbitlang$core$builtin$$Show$to_string$2$(5)} is ${moonbitlang$core$builtin$$Show$to_string$2$(username$moonbit_examples$cmd$factorial$$factorial(5))}`);
})();
