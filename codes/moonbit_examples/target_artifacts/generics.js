...
function username$moonbit_examples$cmd$generics$$Box$new$3$(value) {
  return { value: value };
}
function username$moonbit_examples$cmd$generics$$Box$new$5$(value) {
  return { value: value };
}
(() => {
  const int_box = username$moonbit_examples$cmd$generics$$Box$new$3$(42);
  const str_box = username$moonbit_examples$cmd$generics$$Box$new$5$("Hello Generics");
  moonbitlang$core$builtin$$println$5$(`Int box contains: ${moonbitlang$core$builtin$$Show$to_string$2$(int_box.value)}`);
  moonbitlang$core$builtin$$println$5$(`String box contains: ${str_box.value}`);
})();
